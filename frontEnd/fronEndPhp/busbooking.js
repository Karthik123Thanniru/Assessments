...
function getTrainNumberFromURL() {
    ...
    var trainNumber = urlParams.get("bus_no");
    return trainNumber;
}
function displayBookingDetails(trainNumber) {
    $.ajax({
        url: "http://localhost/test/practiceTest/busBooking.php",
        type: "GET",
        data: { bus_no: trainNumber },
        dataType: "json",
        success: function(response) {
            if (response && response.length > 0) {
                ...
                trainDiv.append($("<p></p>").text("Bus Number: " + response[0].bus_no));
